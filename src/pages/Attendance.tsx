 ...
 * Descripción: Hub de sesiones para coaches. Calendar + selector de sesión.
...
interface AttendeeState {
  ...
  reservationId: string; status: AttendanceStatus | null;
  note: string; attendanceId: string | null;
}
...
export default function AttendancePage() {
  ...
  const [selectedDate, setSelectedDate] = useState<Date>(
    locationState?.preselectedDate ? new Date(locationState.preselectedDate) : new Date()
  );
  ...
  const { isLoading: loadingAttendees } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data: reservations } = await supabase.from('reservations')
        ...
        .eq('session_id', selectedSessionId).eq('reservation_status', 'confirmed');
      const { data: existingAttendance } = await supabase.from('attendance')
        .select('*').eq('session_id', selectedSessionId);
      ...
      (reservations || []).forEach((r: any) => {
        ...
        newAttendees[r.user_id] = {
          userId: r.user_id, fullName: userProfile?.full_name || 'Sin nombre',
          avatarUrl: userProfile?.avatar_url || null, reservationId: r.id,
          status: existing?.attendance_status as AttendanceStatus || null,
          note: existing?.notes || '', attendanceId: existing?.id || null,
        };
      });
  ...
  const toggleAttendance = async (userId: string, status: AttendanceStatus) => {
    ...
    if (previousStatus === status) {
      ...
      const { error } = await supabase.from('attendance').delete()
        .eq('session_id', selectedSessionId).eq('user_id', userId);
      if (error) {
        setAttendees(prev => ({ ...prev, [userId]: { ...prev[userId], status: previousStatus } }));
        toast.error('No se pudo desmarcar la asistencia'); return;
      }
      toast.success('Asistencia desmarcada');
      queryClient.invalidateQueries({ queryKey: ['attendance-session', selectedSessionId] }); return;
    }
    setAttendees(prev => ({ ...prev, [userId]: { ...prev[userId], status } }));
    const { error } = await supabase.from('attendance').upsert({
      session_id: selectedSessionId, user_id: userId, attendance_status: status,
      checkin_time: status === 'present' ? new Date().toISOString() : null,
    }, { onConflict: 'session_id,user_id' });
    if (error) {
      setAttendees(prev => ({ ...prev, [userId]: { ...prev[userId], status: previousStatus } }));
      toast.error('No se pudo guardar la asistencia'); return;
    }
  ...
  const saveSingleAttendance = async (userId: string) => {
    ...
    const { error } = await supabase.from('attendance').upsert({
      session_id: selectedSessionId, user_id: userId, attendance_status: a.status,
  ...
  const saveAllAttendance = async () => {
    ...
    const rows = withStatus.map(a => ({
      session_id: selectedSessionId, user_id: a.userId, attendance_status: a.status,
      notes: a.note ? sanitizeText(a.note) : null,
      checkin_time: a.status === 'present' ? new Date().toISOString() : null,
    }));
  ...
  const handleCommunicate = async () => {
    ...
    const { error } = await supabase.from('notifications').insert(
      userIds.map(uid => ({
        user_id: uid, title: `📢 ${sessionTitle}`, message: sanitized, type: 'announcement',
      }))
    );
    if (error) toast.error('No se pudo enviar el mensaje');
    else {
      toast.success(`✅ Mensaje enviado a ${userIds.length} alumnos`);
      setShowCommunicate(false); setCommunicateMessage('');
    }
  };
  ...
  const handleQRScan = async (result: string) => {
    ...
    if (!match) { toast.error('QR inválido. Probá con el QR de un alumno.'); return; }
    const scannedUserId = match[1];
    const attendee = attendees[scannedUserId];
    if (!attendee) { toast.error('Este alumno no está en la lista de esta sesión.'); return; }
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6 animate-fade-in">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="font-display text-3xl font-extrabold text-foreground flex items-center gap-2">
            <ClipboardCheck size={28} className="text-primary" /> Sesiones
          </h1>
      ...
      <div className="bg-card border border-border rounded-xl p-4">
        ...
        <Calendar
          mode="single" selected={selectedDate} locale={es}
          ...
          <div className="mt-3 flex items-center justify-between flex-wrap gap-2">
            ...
            <div className="flex items-center gap-2">
              <Button variant="outline" size="sm" className="gap-1"
                onClick={() => navigate(`/sesion/${selectedSession.id}`)}>
              ...
              {Object.keys(attendees).length > 0 && (
                <Dialog open={showCommunicate} onOpenChange={setShowCommunicate}>
                  ...
                  <DialogContent className="bg-card border-border">
                    <DialogHeader>
                      <DialogTitle className="font-display">Enviar mensaje a alumnos</DialogTitle>
                    </DialogHeader>
                    <div className="space-y-4">
                      <p className="text-sm text-muted-foreground">
                        Este mensaje se enviará como notificación a los {Object.keys(attendees).length} alumnos de esta sesión.
                      </p>
              ...
              {filteredAttendees.map(attendee => {
                ...
                return (
                  <div key={attendee.userId}
                    className={cn('bg-card border rounded-xl p-4 transition-colors',
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex items-center gap-1">
                        {(['present', 'late', 'absent'] as AttendanceStatus[]).map(s => {
                          ...
                          return (
                            <button key={s} onClick={() => toggleAttendance(attendee.userId, s)}
                              ...
                              className={cn('w-8 h-8 rounded-full border flex items-center justify-center transition-all',
                                attendee.status === s ? cfg.className + ' scale-110' : 'border-border text-muted-foreground hover:border-primary/40')}>
                              <Icon size={14} />
                            </button>
                          );
                        })}
                        <button onClick={() => setEditingNoteFor(editingNoteFor === attendee.userId ? null : attendee.userId)}
                          title="Agregar nota"
                          className={cn('w-8 h-8 rounded-full border flex items-center justify-center transition-all',
                            attendee.note ? 'border-secondary text-secondary bg-secondary/10' : 'border-border text-muted-foreground hover:border-secondary/40')}>
                          <MessageSquare size={13} />
                        </button>
                        <button onClick={() => setQrMember(attendee)} title="Ver QR"
                    ...
                    {editingNoteFor === attendee.userId && (
                      <div className="mt-3 pt-3 border-t border-border">
                        <Textarea placeholder="Feedback, observaciones, rendimiento..."
                          value={attendee.note} onChange={e => updateLocalNote(attendee.userId, e.target.value)}
                        ...
                        <div className="flex justify-end mt-2 gap-2">
                          <Button size="sm" variant="ghost" onClick={() => setEditingNoteFor(null)}
                            className="text-muted-foreground text-xs">Cerrar</Button>
                          <Button size="sm" onClick={() => { saveSingleAttendance(attendee.userId); setEditingNoteFor(null); }}
...
function AssignMemberToSession({
  ...
  const { data: searchResults } = useQuery({
    ...
    queryFn: async () => {
      ...
      return data
        .map((m: any) => ({
          user_id:    m.user_id,
          full_name:  m.users?.profiles?.full_name ?? '',
          avatar_url: m.users?.profiles?.avatar_url ?? null,
        }))
        .filter((p: any) =>
    ...
    enabled: search.length >= 2 && !!clubId,
  });
  ...
  const assignMember = async (userId: string, name: string) => {
    ...
    setAssigning(false);
    if (error) toast.error('No se pudo asignar al alumno');
    else { toast.success(`✅ ${name} asignado a esta sesión`); setSearch(''); onAssigned(); }
  };
    ...
    <div className="bg-card border border-dashed border-border rounded-xl p-4">
      ...
      {searchResults && searchResults.length > 0 && (
        <div className="mt-2 space-y-1">
          {searchResults.map((p: any) => (
            <button key={p.user_id} onClick={() => assignMember(p.user_id, p.full_name)} disabled={assigning}
      ...
      {noResults && (
        <div className="mt-3 space-y-3">
          <p className="text-sm text-muted-foreground">No se encontraron alumnos en tu club</p>
          {!showCreateMember ? (
            <Button variant="outline" size="sm" className="gap-1 w-full"
              onClick={() => { setShowCreateMember(true); setNewMemberName(search); }}>
              <UserPlus size={14} /> Crear alumno nuevo
            </Button>
          ) : (
            <div className="space-y-2 p-3 bg-muted/30 rounded-lg border border-border">
              <Label className="text-xs">Nombre del nuevo alumno</Label>
              <Input value={newMemberName} onChange={e => setNewMemberName(e.target.value)}
                placeholder="Nombre completo" className="bg-background border-border" />
