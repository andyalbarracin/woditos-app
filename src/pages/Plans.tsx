 ...
 *   v1.1: Free con analytics básicas disponibles + 1 coach límite.
 *         Pro con multi-coach ya disponible. Tabla de comparación ajustada a realidad.
 */
...
const PLANS: Plan[] = [
  {
    ...
    features: [
      ...
      { text: 'Analytics básicas',            included: true  },
      { text: 'Multi-coach',                  included: false },
  ...
  },
  {
    ...
    features: [
      ...
      { text: 'Multi-coach (hasta 3)',         included: true,  soon: false },
      { text: 'Analytics avanzadas',          included: true,  soon: true  },
...
export default function Plans() {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 animate-fade-in">
      ...
      {/* Aclaración sobre "pronto" */}
      <div className="bg-muted/30 border border-border rounded-xl px-5 py-4">
        <p className="text-xs text-muted-foreground leading-relaxed">
          <span className="font-semibold text-foreground">¿Qué significa "pronto"?</span>
          {' '}Las features marcadas están diseñadas y en desarrollo activo.
          Durante la beta, el Plan Pro incluye acceso anticipado a todas ellas cuando se lancen,
          sin costo adicional.
        </p>
      </div>
      ...
      <div className="bg-card border border-border rounded-xl overflow-hidden">
        <div className="px-6 py-4 border-b border-border">
          ...
          <p className="text-xs text-muted-foreground mt-1">
            ✓ disponible hoy &nbsp;·&nbsp; <span className="opacity-60">pronto</span> = en desarrollo &nbsp;·&nbsp; — no incluido en este plan
          </p>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {[
                { label: 'Sesiones y agenda',         row: [true,  true,  true,  true ], proSoon: false },
                { label: 'Reservas y asistencia QR',  row: [true,  true,  true,  true ], proSoon: false },
                { label: 'Feed social y comunidad',   row: [true,  true,  true,  true ], proSoon: false },
                { label: 'Módulo de rutinas',         row: [true,  true,  true,  true ], proSoon: false },
                { label: 'Feedback post-sesión',      row: [true,  true,  true,  true ], proSoon: false },
                { label: 'Biblioteca de ejercicios',  row: [true,  true,  true,  true ], proSoon: false },
                { label: 'Analytics básicas',         row: [true,  true,  true,  true ], proSoon: false },
                { label: 'Multi-coach',               row: [false, true,  true,  true ], proSoon: false },
                { label: 'Analytics avanzadas',       row: [false, true,  true,  true ], proSoon: true  },
      ...
      <div className="text-center space-y-2 pb-4">
        ...
        <p className="text-xs text-muted-foreground/60">
          Precios en USD. Facturación mensual. Sin contratos de permanencia.
        </p>
      </div>
    </div>
  );
...
function PlanCard({ plan, currentPlanId }: { plan: Plan; currentPlanId: string }) {
  const isCurrent = plan.id === currentPlanId ||
    (plan.id === 'pro' && currentPlanId !== 'pro_plus' && currentPlanId !== 'enterprise');
  ...
  return (
    <div className={`relative rounded-xl border p-5 flex flex-col gap-4 transition-all ${
      ...
      <div className="space-y-1 pt-1">
        ...
        <p className={`text-xs ${plan.disabled ? 'text-muted-foreground/40' : 'text-muted-foreground/70'}`}>
          {plan.coaches}
        </p>
      </div>
        ...
        {plan.features.slice(0, 7).map((f, i) => (
          <li key={i} className={`flex items-center gap-2 text-xs ${
            ...
            <span className={`shrink-0 ${
              f.included
                ? plan.disabled ? 'text-muted-foreground' : f.soon ? 'text-muted-foreground/50' : 'text-secondary'
                : 'text-muted-foreground'
            }`}>
              {f.included ? <Check size={12} /> : <span className="text-[10px]">—</span>}
            </span>
            <span className={`flex-1 ${
              f.soon ? 'text-muted-foreground/60' : f.included ? 'text-foreground/80' : 'text-muted-foreground/60'
            }`}>
              {f.text}
            </span>
