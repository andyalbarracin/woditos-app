...
import {
  ...
  UserCheck, BarChart3, Activity, Link, Trash2, UserMinus, ListChecks,
} from 'lucide-react';
...
export default function CoachDashboard() {
  ...
  const { data: monthSessions } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from('sessions')
        ...
        .gte('start_time', startOfMonth(selectedDay).toISOString())
        .lte('start_time', endOfMonth(selectedDay).toISOString())
        .eq('coach_id', user!.id);
      return data || [];
    },
  });
  ...
  const daysWithSessions = useMemo(() =>
    (monthSessions ?? []).map((s: any) => new Date(s.start_time)),
  [monthSessions]);
  ...
  const { data: members } = useQuery({
    ...
    queryFn: async () => {
      const { data: myClub } = await supabase
        .from('club_memberships').select('club_id').eq('user_id', user!.id).limit(1).single();
      if (!myClub) return [];
      const { data } = await supabase.from('club_memberships')
        ...
        .eq('club_id', myClub.club_id).eq('status', 'active');
      return data || [];
    },
  });
  ...
  const invalidateSessionQueries = () => {
    ['coach-day-sessions','coach-month-sessions','coach-all-sessions',
     'coach-attendance-stats','my-coach-sessions','unassigned-sessions',
     'next-session','upcoming-sessions-dashboard'].forEach(k =>
      queryClient.invalidateQueries({ queryKey: [k] }));
  };

  const notifyMembers = async (session: any, title: string, message: string) => {
    const confirmed = session.reservations?.filter((r: any) => r.reservation_status === 'confirmed') || [];
    if (!confirmed.length) return;
    await supabase.from('notifications').insert(
      confirmed.map((r: any) => ({ user_id: r.user_id, title, message, type: 'session' }))
    );
  };

  const canDelete = (s: any) =>
    s.created_by === user?.id || (!s.created_by && s.coach_id === user?.id) || user?.role === 'super_admin';
  ...
  const unclaimMutation = useMutation({
    mutationFn: async (session: any) => {
      await notifyMembers(session, '📋 Coach desasignado',
        `El coach se desasignó de "${session.title}" del ${format(new Date(session.start_time), "d MMM 'a las' HH:mm", { locale: es })}.`);
      const { error } = await supabase.from('sessions').update({ coach_id: null }).eq('id', session.id);
      if (error) throw error;
    },
    onSuccess: () => { invalidateSessionQueries(); toast.success('Sesión soltada'); },
    onError: () => toast.error('No se pudo soltar la sesión'),
  });

  const deleteMutation = useMutation({
    mutationFn: async (session: any) => {
      await notifyMembers(session, '❌ Sesión cancelada',
        `La sesión "${session.title}" del ${format(new Date(session.start_time), "d MMM 'a las' HH:mm", { locale: es })} fue cancelada.`);
      const { error } = await (supabase.rpc as any)('delete_session_cascade', { p_session_id: session.id });
      if (error) throw error;
    },
    onSuccess: () => { setSessionToDelete(null); invalidateSessionQueries(); toast.success('Sesión eliminada'); },
    onError: (err: any) => toast.error('No se pudo eliminar: ' + (err.message || '')),
  });

  const markAttendance = useMutation({
    mutationFn: async ({ sessionId, userId, status, currentStatus }: {
  ...
  const sessionsByType = allSessions?.reduce((acc: any[], s: any) => {
    const ex = acc.find(x => x.tipo === s.session_type);
    if (ex) ex.cantidad++; else acc.push({ tipo: s.session_type, cantidad: 1 });
    return acc;
  }, []) || [];

  const attendancePie = allAttendance?.reduce((acc: any[], a: any) => {
    const label = a.attendance_status === 'present' ? 'Presente' : a.attendance_status === 'late' ? 'Tarde' : 'Ausente';
    const ex = acc.find(x => x.name === label);
    if (ex) ex.value++; else acc.push({ name: label, value: 1 });
    return acc;
  }, []) || [];

  const weeklyData = allSessions?.reduce((acc: any[], s: any) => {
    const week = format(new Date(s.start_time), "'Sem' w", { locale: es });
    const ex = acc.find(x => x.semana === week);
    const attended = s.attendance?.filter((a: any) => a.attendance_status === 'present').length || 0;
    if (ex) { ex.sesiones++; ex.asistentes += attended; }
    else acc.push({ semana: week, sesiones: 1, asistentes: attended });
    return acc;
  }, []) || [];

  const totalMembers    = members?.length || 0;
  const totalSessions30 = allSessions?.length || 0;
    ...
    <div className="max-w-5xl mx-auto space-y-6 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          <Button variant="outline" onClick={() => navigate('/rutinas/nueva')} className="gap-2">
            <ListChecks size={16} /> Nueva Rutina
          </Button>
      ...
      <Tabs defaultValue="today">
        <TabsList className="bg-card border border-border grid grid-cols-2 sm:grid-cols-5 h-auto gap-1 p-1">
          ...
          <TabsTrigger value="analytics" className="gap-1.5 data-[state=active]:bg-accent data-[state=active]:text-accent-foreground">
        ...
        <TabsContent value="today" className="mt-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="bg-card border border-border rounded-xl p-4">
              <p className="text-xs text-muted-foreground uppercase tracking-wider font-bold mb-3">Seleccioná un día</p>
              <CalendarWidget
                mode="single" selected={selectedDay} locale={es}
                onSelect={d => { if (d) setSelectedDay(d); }}
                className="rounded-md border-0 pointer-events-auto w-full"
                classNames={{
                  months: "flex flex-col w-full", month: "space-y-3 w-full",
            ...
            <div className="space-y-3">
              ...
              {daySessions && daySessions.length > 0 ? daySessions.map((s: any) => {
                ...
                return (
                  <div key={s.id} className="bg-card border border-border rounded-xl p-4">
                        ...
                        {s.coach_id === user?.id && (
                          <>
                            <Button variant="ghost" size="icon" className="h-7 w-7 text-muted-foreground hover:text-foreground"
                              title="Soltar" disabled={unclaimMutation.isPending}
                              onClick={() => unclaimMutation.mutate(s)}>
                              <UserMinus size={14} />
                            </Button>
                            {canDelete(s) && (
                              <Button variant="ghost" size="icon" className="h-7 w-7 text-muted-foreground hover:text-destructive"
                                title="Eliminar" onClick={() => setSessionToDelete(s)}>
                                <Trash2 size={14} />
                              </Button>
                            )}
                          </>
                          ...
                          return (
                            <div key={r.id} className="flex items-center justify-between py-1">
                              <span className="text-sm text-foreground truncate flex-1 mr-2">
                                {r.users?.profiles?.full_name || 'Sin nombre'}
                              </span>
                    ...
                    {confirmed.length === 0 && <p className="text-xs text-muted-foreground">Sin reservas aún</p>}
                  </div>
                );
              }) : (
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-border">
                  {['Miembro','Nivel','Estado'].map(h => (
                    <th key={h} className="text-left px-4 py-3 text-xs text-muted-foreground uppercase tracking-wider font-bold">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {members && members.length > 0 ? members.map((m: any) => {
                  ...
                  return (
                    <tr key={m.id} className="border-b border-border last:border-0 hover:bg-muted/50">
                      ...
                      <td className="px-4 py-3"><span className="text-xs capitalize text-muted-foreground">{mp?.experience_level || '-'}</span></td>
                      <td className="px-4 py-3"><span className="text-xs capitalize text-secondary">{m.membership_status}</span></td>
                    </tr>
                  );
                }) : (
                  <tr><td colSpan={3} className="px-4 py-8 text-center text-muted-foreground">No hay miembros</td></tr>
                )}
              </tbody>
            </table>
          </div>
                ...
                <ResponsiveContainer width="100%" height={220}>
                  <PieChart>
                    <Pie data={attendancePie} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={80}
                      ...
                      {attendancePie.map((_: any, i: number) => <Cell key={i} fill={PIE_COLORS[i % PIE_COLORS.length]} />)}
                    </Pie>
                    <RechartsTooltip />
                  </PieChart>
                </ResponsiveContainer>
          ...
          {sessionToDelete && (
            <div className="space-y-4">
              <p className="text-sm text-foreground">
                ¿Eliminar <strong>"{sessionToDelete.title}"</strong>?
              </p>
              ...
              {confirmedCount(sessionToDelete) > 0 && (
                <p className="text-sm text-destructive/80 bg-destructive/10 rounded-lg px-3 py-2">
                  Se notificará a {confirmedCount(sessionToDelete)} inscripto{confirmedCount(sessionToDelete) !== 1 ? 's' : ''}.
                </p>
              )}
              <p className="text-xs text-muted-foreground">Esta acción no se puede deshacer.</p>
              <div className="flex justify-end gap-2 pt-2">
                <Button variant="outline" onClick={() => setSessionToDelete(null)}>Cancelar</Button>
